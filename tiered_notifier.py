...
class NtfyNotificationTier(NotificationTier):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.tags = self.config.get("tags", "computer,claude")
...
class TieredNotifier:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        macos_config = self.config.tier_configs.get("macos", {})
    ...
    def send_notification(self, title: str, message: str, tier: Optional[str] = None) -> bool:
        ...
        if not notification_tier.is_available():
            print(f"Warning: Notification tier '{tier}' is not available", file=sys.stderr)
            return False
        
        return notification_tier.send_notification(title, message)
    
    def send_tiered_notification(self, title: str, message: str, session_id: Optional[str] = None) -> bool:
        """Send immediate notifications and schedule delayed ones"""
        success = False
        
        # Send immediate tiers (not in delayed_tiers)
        immediate_tiers = [t for t in self.config.enabled_tiers if t not in self.config.delayed_tiers]
    ...
    def _schedule_delayed_notifications(self, title: str, message: str, session_id: str):
        """Schedule delayed notifications that fire if session stays idle"""
        
        # Create a separate script to handle the delay
        delay_script = f"""#!/usr/bin/env -S uv run python
import time
import json
import requests
from pathlib import Path
    ...
    try:
        for tier_config in {json.dumps([(name, self.config.tier_configs.get(name, {})) for name in self.config.delayed_tiers])}:
            ...
            if tier_name == "ntfy":
                ...
                tags = config.get("tags", "computer,claude")
...
"""
        ...
        subprocess.Popen([delay_script_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
